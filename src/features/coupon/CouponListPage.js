...
export function CouponListPage(props) {
  ...
  useEffect(() => {
    if (type == 99) {
      axios.get('/api/coupon/available', { params: { limit: 25 } }).then(res => {
        if (res.status === 200) {
      ...
      axios.get('/api/coupon/whitelist', { params: { limit: 25 } }).then(res => {
        if (res.status === 200) {
          console.log(res.data.data)
          setData(res.data.data)
        }
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.couponListContainer}>
        {data.map(child => {
          return (
            <VoucherCard key={child._id} coupon={child} myCouponList={myCouponList} setMyCouponList={setMyCouponList} />
          )
        })}
      </div>
    </div>
