...
function VoucherListVertical(props) {
  ...
  useEffect(() => {
    axios
      ...
      .catch(e => {
        console.log(e)
        alert('Không thể tải danh sách ưu đãi của bạn. Vui lòng liên hệ admin để được hỗ trợ nhanh nhất')
      })
  }, [])
  ...
  return (
    <div className={styles.voucherList}>
      <div className={styles.listVertical}>
        {data.map(child => {
          console.log(child)
          return <VoucherCardVertical key={child._id} coupon={child.coupon} isUsed={child.isUsed ? 1 : 0} />
        })}
      </div>
    </div>
  )
