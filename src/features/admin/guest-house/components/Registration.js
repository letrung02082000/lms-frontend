          ...
          return (
            <div className={styles.roomContainer} key={child._id}>
              <div className={styles.userInfoContainer}>
                <div>
                  <p>Tên:</p>
                  <textarea defaultValue={child?.name} rows={1} id={`formName_${child._id}`} />
                </div>
                <div>
                  <p>Số điện thoại:</p>
                  <textarea defaultValue={child?.tel} rows={1} id={`formTel_${child._id}`} />
                </div>
                <div>
                  <p>Ngày vào:</p>
                  {/* <Datetime
                ...
                <div>
                  ...
                  <select value={child.guestHouse._id} onChange={e => handleRoomChange(child._id, e.target.value)}>
                    {rooms.map(child => {
                      return <option value={child._id}>Phòng {child.number}</option>
                    })}
                  </select>
                </div>
                <div>
                ...
                <div>
                  <p style={{ margin: '1rem 0' }}>
                    ...
                    {child.guestHouse?.isAvailable ? 'Còn phòng' : 'Không có sẵn'}
                  </p>
                </div>
              </div>
              <div className={styles.buttonContainer}>
                ...
                <button
                  style={child.state === 3 ? { backgroundColor: 'var(--primary)', color: 'white' } : null}
                  onClick={() => handleAddRoomButton(child._id, child.guestHouse._id)}
                >
                  Đã nhận phòng
                </button>
                <button
