          ...
          {data.map((child, index) => {
            return (
              <div className={styles.itemContainer} key={child._id}>
                <textarea id={`formNewName_${child._id}`}>{child.name}</textarea>
                <textarea id={`formNewDesc_${child._id}`}>{child.description}</textarea>
                <div className={styles.buttonContainer}>
                  <button onClick={() => handleUpdateButton(child._id)}>Cập nhật</button>
                  {child.isVisible ? (
                    <button onClick={() => toggleVisibleButton(child._id, !child.isVisible)}>Ẩn</button>
                  ) : (
                    <button
                      style={{
                        color: 'red',
