...
function GuestHouseBillPage() {
  ...
  const price = state.rentType === 0 ? roomInfo.dailyPrice : roomInfo.monthlyPrice
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.roomInfoContainer}>
        ...
        <p>Tình trạng: {roomInfo.state == 0 ? 'Còn phòng' : 'Không thể đặt phòng này'}</p>
        <p>Đơn giá: {state.rentType == 0 ? roomInfo.dailyPrice : roomInfo.monthlyPrice} vnđ</p>
        <p>Hình thức thuê: {state.rentType == 0 ? 'Theo ngày' : 'Theo tháng'}</p>
        <p>
          Thời gian: {state.duration} {state.rentType == 0 ? 'ngày' : 'tháng'} (Bạn có thể gia hạn khi gần hết hạn)
        </p>
        <p>
          Ngày bắt đầu:{' '}
